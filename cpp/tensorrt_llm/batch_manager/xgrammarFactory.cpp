...
template <typename T>
T parseEnvVar(char const* envVal, T defaultValue)
{
    if (!envVal)
    {
        return defaultValue;
    ...
    tle::GuidedDecodingConfig const& guidedDecodingConfig, const SizeType32 vocabSizePadded)
{
    ...
    int const max_threads = parseEnvVar(std::getenv("XGR_MAX_THREADS"), 8);
    bool const cache_enabled = parseEnvVar(std::getenv("XGR_CACHE_ENABLED"), false);
    long long const max_mmemory_bytes = parseEnvVar(std::getenv("XGR_MAX_MEMORY_BYTES"), -1);
    ...
    if (tokenizerStr)
    {
        ...
        picojson::object const& obj = v.get<picojson::object>();
        ...
        auto const& tokenizerInfo = xgrammar::TokenizerInfo(guidedDecodingConfig.getEncodedVocab().value(), vocab_type,
            vocabSizePadded, guidedDecodingConfig.getStopTokenIds(), add_prefix_space);
        mXGrammarCompiler
            = std::make_shared<xgrammar::GrammarCompiler>(tokenizerInfo, max_threads, cache_enabled, max_mmemory_bytes);
    }
    else
    {
...
std::shared_ptr<IGrammarMatcher> XGrammarMatcherFactory::Create(
