...
LlgTokenizerPtr llgCreateTokenizer(
    tle::GuidedDecodingConfig const& guidedDecodingConfig, tensorrt_llm::runtime::SizeType32 vocabSize)
{
    ...
    char error_buf[1024];
    auto tokenizer
        = std::shared_ptr<LlgTokenizer>(llg_new_tokenizer(&tok_init, error_buf, sizeof(error_buf)), llg_free_tokenizer);
    TLLM_CHECK_WITH_INFO(tokenizer, "error creating tokenizer: %s, error_buf");

    return tokenizer;
}

bool llgValidateGrammar(
    tle::GuidedDecodingParams const& guidedDecodingParams, LlgTokenizerPtr tokenizer, std::string* errorMessage)
{
