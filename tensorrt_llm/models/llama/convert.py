...
def capture_activation_range(model,
    ...
    for data in tqdm(calib_dataloader, desc="calibrating model"):
...
def get_tokenizer(ckpt_path, trust_remote_code=True, model_type=None):
...
def quantize(hf_model_dir: str,
    ...
    tokenizer = get_tokenizer(hf_model_dir, trust_remote_code=trust_remote_code)
    dataloader = get_calib_dataloader(dataset_name_or_dir=calib_dataset,
                                      tokenizer=tokenizer,
                                      config=calib_config)

    act_range = capture_activation_range(hf_model, dataloader)
    qkv_para, smoother = {}, {}
    if use_smooth_quant:
